...
def main():
    if len(sys.argv) < 2 or sys.argv[1] == '-h':
        ...
        print(f"  {Fore.YELLOW}-cmp{Style.RESET_ALL}    - 취약성, 악성, 개방 포트 정보를 조합하여 출력")
        print(f"  {Fore.YELLOW}-mpc{Style.RESET_ALL}    - 악성, 개방 포트, 취약성 정보를 조합하여 출력")
        print(f"\n예시:")
        print(f"  python script_name.py init <api_key>")
        print(f"  python script_name.py ip 1.1.1.1 -a")
        print(f"  python script_name.py ip 8.8.8.8 -cmp\n")
        return
